...
function checkFieldLength(fieldName: string, value: string | null | undefined, maxLength: number): string {
	if (!value) return ''
	if (value.length > maxLength) {
		console.warn(`Field '${fieldName}' exceeds ${maxLength} characters. Value: ${value.substring(0, 50)}...`)
			...
			for (const game of prismGames) {
				...
				if (!exists) {
					try {
						...
						console.log('game skipped', steamAppid)
					}
				}
			}
